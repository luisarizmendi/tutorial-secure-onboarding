...
Bur first, let's start from the begening and let's install the Image Builder.
...
You need first to create the blueprint file where you include the image customizations and the use that file to create the actual OSTree repository.
...
You can customize the image created by the Image Builder throught a `blueprint.toml` file. You can check https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/9/html/composing_a_customized_rhel_system_image/index[how to use blueprint in Image Builder] but take into account that not all variables are applied with creating OSTree images, so it's better to review the https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/9/html-single/composing_installing_and_managing_rhel_for_edge_images/index#image-customizations_composing-a-rhel-for-edge-image-using-image-builder-command-line[supported blueprint configurations for RHEL for Edge] documentation.  
...
If you want to include individial packages you can use the `[[packages]]` section.
...
An additional comment about the users in OSTree RHEL. If you went throught the xref:02-rfe-intro.adoc#rfe-intro-article[OSTree-based Operating Systems article] shared in the previous module, you noticied that in OSTree RHEL you will have two "kinds" of users, the ones created on the first boot of the image, and the ones generated by day-2 operations.The users created by the Image Builder could be described these "initial" users that are created during the first boot of the device, and which target is to perform the onboarding configurations and as administrative access to the devices.
...
If you plan to use the provided example, you just need to download the Blueprint template and addapt it to your environment by changing certain values on it.
...
Now is time to create the OSTree image. You will need point to the name that you gave to the Blueprint (the one that you used in the `name` parameter in the Blueprint file), not the file name as you did in the previous step:
...
Also a good idea is to tag the last conatiner image as "latest":
...
Remember that if you are using this VM to run other services, for example the FDO services, you need to be sure that you don't use same ports (use tcp/8090 to be sure that it does not interfiere with FDO default ports)
...
You can double check that your conatiner is running and publishing the contents by using `CURL`. If you execute this command from the same VM that is publishing the OSTree repo you could use `localhost` as the IP, but it is always better to specifify the actual IP address of the system: 
...
For the FDO manufacturing server URL, you will need the IP address and the port (if you used the defaults, for the manufacturing server is `8080`). Remember that the VM where the FDO manufacturing server will depend on the option that you choosed to deploy the FDO servers: All-in-one VS distributed services.
...
Same that you did while creating the OSTree image, you will need to use this Blueprint to create the new image, but in this case, you will also need to include information about where the OSTree repo is published.

You should have already configured at this point two variables with the IP address and the TCP port where the repo is published: `${IMAGE_BUILDER_IP}` and `${IMAGE_PUBLISH_PORT}`, but you will also need to include the path (URL) to the right content which depend on the RHEL release and architecture used to generate the OSTRee image.
...
Now that you have all required values, run the actual command in the Image Builder that will create the ISO file with the embed OSTree repository:
