...
Once you have `libvirt` or any other hypervisor installed, and your RHEL ISO downloaded, you can create a VM with *2 vCPUs, 4 GB memory and 50 GB disk* using the "Local install media" option selecting the RHEL ISO. You can use `virt-manager` in case of `libvirt` or running the following command to create the VM:
