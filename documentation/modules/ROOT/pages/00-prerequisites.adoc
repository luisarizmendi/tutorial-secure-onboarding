...
If you want to save some time during the lab, also download the latest RHEL ISO.
...
If your hypervisor system has enough CPU and RAM, you probably would like to use a more realistic architecture, where the FDO services are running in dedicated servers. In this case, you will still need the VMs shown in "Option 1" (and to download the RHEL ISO), but you will need to run three additional VMs for FDO servers.
