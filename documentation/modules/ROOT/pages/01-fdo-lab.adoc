...
The onboarding automation will register your system in the https://console.redhat.com[Red Hat Hybrid Console] so the file will need you Red Hat account credentials. In order to not include the user and password directly in plain text, the script that I created accepts the inputs as base64 (for a production environment you should write a script that accept encrypted passwords, not just with base64).
