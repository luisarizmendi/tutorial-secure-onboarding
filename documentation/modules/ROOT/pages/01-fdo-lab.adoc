...
FDO is an specification that has been implemented by multiple vendors. In this tutorial we are going to use Red Hat development which is published upstream in https://github.com/fedora-iot/fido-device-onboard-rs[this GitHub repository]. 
...
In this option you will have the Image Buider server and 3 additional FDO VMs (manufacturing, rendezvours and owner). If you find that architecture requires too much resources but you still want to test the FDO splited into different servers, you could install one of those (maybe the manufacturing server) in the same VM than the Image Builder in order to save some CPU and Memory.
...
If you remember, during the manufacturing phase you have to “inject” certain certificates and keys into your device. When using the `fdo-aio` service those are generated automatically, but now you are going to create them manually.
...
[source,bash,subs="+macros,+attributes"]
----
sudo for i in manufacturer owner device-ca diun ; do
...
It's time to configure the Manufacturing server. You can find an example configuration in `/usr/share/doc/fdo/manufacturing-server.yml`. 

Create your own configuration in the `/etc/fdo` directory using that file as a template. Here you can find another configuration example:
...
Next, create the configuration file where you include the path to the manufacturer certificate (if you followed the steps shown above, it shoudl be in the `/etc/fdo/keys directory). I created this file (located in `/etc/fdo/rendezvous-server.yml`) from the example that you can find in `/usr/share/doc/fdo/rendezvous-server.yml`:
...
This service will be configured in the Owner sever, but since that's an step that is needed also when you install all FDO services on the same node, it will explained more in detail in the next section.
...
There is an example of this located in `/usr/share/doc/fdo/serviceinfo-api-server.yml` that can be used as a template to create your own configuration file. You can also review the default configuration (in this case is for the all-in-one setup, you can know it by taking a look to the stores path):
...
It's time to configure your own automation use case, so prepare your files and scripts before moving on! ... or xref:01-fdo-lab.adoc#fdo-optionb[jump into the section] where it is explained how to use the onboarding automation example.
...
We start with the initial user. The REHL image only has configure the root user, and it has no password by default so you need to include an additional user that you can use to access the device by using a public SSH key.
...
The next section in the configuration is `files`, which can be used to transfer file to the devices. You need to include the source and destination paths along with the permissions to be setup in the device.
...
Just remember that at this moment https://github.com/fedora-iot/fido-device-onboard-rs/issues/290[you cannot use constracts such as >> or && in the commands]. If you "need" to use constructs you can create a bash script file, copy and make it executable using the `files` section and then run that script (`commands` section). 

In order to configure the section, you will need to include the `command` followed by the `args` each on one line. You can also include a optional boolean variables:

* `may_fail`: makes possible to continue with the onboarding even if this command fails (default is false)

* `return_stdout`: the command will return the standard output (default is false).

* `return_stderr`: has the same effect than `return_stdout` but with the errors (default is false).


This would be an example with all the variables, including the optionals:
...
If configured, it eill perform a disk encryption using https://wiki.archlinux.org/title/Clevis[Clevis].

You need to include at least which disk or patition you want to encrypt (`disk_label`), the encryption method (`pin`).
...
If you don't want to use your home directory, copy the files into another definitive location, for example `/etc`:
...
The example Serviceinfo API file doesn't have the right tokens, so update it with the ones on your system. Be sure that you use the commands for the FDO deployment option that you selected (all-in-one versus dedicated servers) since the directory where you find `serviceinfo_api_server.yml` file is different
...
[source,bash,subs="+macros,+attributes"]
----
SERVICE_TOKEN=$(grep service_info_auth_token /etc/fdo/serviceinfo-api-server.yml | awk '{print $2}')
ADMIN_TOKEN=$(grep admin_auth_token /etc/fdo/serviceinfo-api-server.yml | awk '{print $2}')
----
--
====
...
[source,bash,subs="+macros,+attributes"]
----
sudo cp -f serviceinfo_api_server.yml serviceinfo-api-server.yml
----
--
====
